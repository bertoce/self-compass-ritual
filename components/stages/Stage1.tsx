...
// Hierarchy level 2 — section navigation label
const SectionLabel = ({ text, count, max }: { text: string; count: number; max: number }) => (
  <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 12 }}>
    <span style={{ fontSize: 9, letterSpacing: '0.2em', textTransform: 'uppercase', fontWeight: 300, color: '#6B6458' }}>
      {text}
    </span>
    <span style={{ fontSize: 9, letterSpacing: '0.1em', color: count > 0 ? '#B8844A' : '#C4B9AB' }}>
      {count} / {max}
    </span>
  </div>
);
...
export default function Stage1({ onComplete }: Stage1Props) {
  ...
  return (
    <div className="stage-enter">
      ...
      >
        {canContinue ? 'See the gap' : 'Select at least one from each section'}
      </button>
